...
import com.myeotra.driver.ui.bottomsheetdialog.cancel.CancelDialogFragment;
...
//        btWaitingTime.setVisibility(View.GONE);
        return view;
    }
    ...
    private void init() {
        ...
        Log.e(TAG, "init : " + new Gson().toJson(data));
        tripResponse = MvpApplication.tripResponse;
        Log.e(TAG, "tripResponse : " + new Gson().toJson(tripResponse));
        ...
        if (data != null && data.getStatus() != null) {
            ...
            if (tripResponse != null && tripResponse.getProviderDetails() != null) {
                ...
                currentLocation = new LatLng(data.getDLatitude(), data.getDLongitude());
    ...
    public void onSuccess(Object object) {
        hideLoading();
        Log.e("data", "statusUpdate StatusFlow res: " + new Gson().toJson(object));
    ...
    public void secondSplitUp(long biggy, TextView tvTimer) {
        ...
        int mins = sec / 60;
        sec = sec - mins * 60;
        tvTimer.setText("Waiting Time : " + String.format("%02d:", hours)
    ...
    private void showAddTollDialog() {
        ...
        EditText toll_amount = view.findViewById(R.id.toll_amount);
        rdogrptoll = view.findViewById(R.id.rdogrptoll);


        rdogrptoll.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                selectedRadioButton = radioGroup.findViewById(i);

                if (selectedRadioButton.getText().toString().equalsIgnoreCase("Yes")) {
                } else {
                    tollAmount = Double.valueOf("0");
...
//        currency_type.setText(Constants.Currency);
        ...
        submitBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (selectedRadioButton != null) {
                    if (selectedRadioButton.getText().toString().equalsIgnoreCase("Yes")) {
                        if (!toll_amount.getText().toString().trim().equalsIgnoreCase("0")) {
                            if (!toll_amount.getText().toString().trim().isEmpty() && toll_amount.getText().toString().trim().length() > 0) {
                                tollAmount = Double.valueOf(toll_amount.getText().toString());
                                customHandler.removeCallbacks(updateTimerThread);
                                statusUpdateCall(STATUS);
                                addTollDialog.dismiss();
                            } else {
                                Toasty.error(thisContext, "Please enter toll", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toasty.error(thisContext, "Please select no", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        tollAmount = Double.valueOf("0");
                        addTollDialog.dismiss();
                    ...
                    Toasty.error(thisContext, "Please select toll yes or no", Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        dismiss.setOnClickListener(v -> {
            ...
            if (selectedRadioButton != null) {
                if (selectedRadioButton.getText().toString().equalsIgnoreCase("Yes")) {
                    Toasty.error(thisContext, "Please select no button", Toast.LENGTH_SHORT).show();
                } else {
                    tollAmount = Double.valueOf("0");
                    addTollDialog.dismiss();
                    statusUpdateCall(STATUS);
