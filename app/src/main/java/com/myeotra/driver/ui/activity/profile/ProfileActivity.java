...
import com.myeotra.driver.ui.activity.change_password.ChangePasswordActivtiy;
...
//        setCountryList();
    ...
    private void setListener() {
        mCountryPicker.setListener((name, code, dialCode, flagDrawableResID) -> {
            txtPhoneNumber.setText(dialCode);
        ...
        txtPhoneNumber.setOnClickListener(v -> mCountryPicker.show(getSupportFragmentManager(), "COUNTRY_PICKER"));
    ...
    @OnClick({R.id.btnSave, R.id.lblChangePassword, R.id.imgProfile, R.id.qr_scan/*, R.id.txtPhoneNumber*/, R.id.menu})
...
//            fbOtpVerify(countryCode, countryDialCode, "");
    ...
    public void onSuccess(UserResponse user) {
        ...
        Log.e(TAG, "profile : res : " + new Gson().toJson(user));

        List<Country> countryList = Country.getAllCountries();

        Log.e(TAG, "onSuccess: " + new Gson().toJson(countryList));

        for (int i = 0; i < countryList.size(); i++) {
            if (countryList.get(i).getDialCode().equalsIgnoreCase(user.getCountryCode())) {
                countryImage.setImageResource(countryList.get(i).getFlag());
                countryNumber.setText(countryList.get(i).getDialCode());
            }
        }
        ...
        qrCodeUrl = !TextUtils.isEmpty(user.getQrcodeUrl()) ? BuildConfig.BASE_URL + user.getQrcodeUrl() : null;
